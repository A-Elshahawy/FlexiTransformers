...
class RotaryMultiHeadAttention(AbstractAttention):
    """
    Applies multi-headed attention with rotation on the query and key.
    ...
    d_model : int
        Model dimension.
    rope_percentage : float, optional
        Percentage of the model dimension to use for rotation. Default: 0.5
    dropout : float, optional
        Dropout probability. Default: 0.1

    References
    ----------
    .. [1] https://arxiv.org/abs/2104.06999
    """
...
class ALiBiMultiHeadAttention(AbstractAttention):
    """
    Multi-headed attention with ALiBi PE.

    ALiBi PE is a family of PE functions that are designed to be more efficient than
    the Rotary PE used in RoPE. There are two major differences:

    1. ALiBi PE is more efficient in terms of computation and memory.
    2. ALiBi PE does not require any additional learnable parameters.

    The original implementation of ALiBi PE is in the `transformers` library, and
    the algorithm is described in the paper "ALiBi: A Family of Fast and Efficient
    Positional Encoding Methods for Language Models" by Press et al.

    References
    ----------
    .. [1] https://arxiv.org/abs/2109.07498
    """
